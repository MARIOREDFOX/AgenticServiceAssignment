...
SAMPLE_TICKETS = [
    ...
    {
        "short_description": "INFOR - Can't open the details on item level in any facility or browser",
        "description": "Hello, since months ago I can't open the details of items in my INFOR. This is the case in any browser (edge or chrome), any facility, but ONLY in PRD. I've done cache clearing, log off log in, all those basic activities to no success. INFOR (Magdalena Wrona) once tried to help me with it with no success. I need this for my daily job, so can you please analyze this topic",
        "priority": "3",
    },
    {
        "short_description": "HAVI SPAIN - KISOFT - DC MADRID SAN FERNANDO - VERY URGENT RESET KISOFT USER PICKJOB 000000079217",
        "description": "Buenos días, por favor, necesitamos que se resetee al usuario de kisoft HGZ. Se ha quedado bloqueado en el pickjob 000000079217 y no podemos loguear su usuario de nuevo para cancelar el pickjob. Hay un camión esperando para cargar este cliente y realizar su entrega. Por favor, asignar este ticket al equipo de Wilson Fonseca, Sofía Cruz, Inés Sardinha o Francisco Guerra para gestionar con KNAPP. Gracias",
        "priority": "3",
    },
    {
        "short_description": "DIMS error",
        "description": "Hello, Good morning , Can you please help me fix DIMS error, as of this time i cannot access DIMS, please see below occurred error. Thank you",
        "priority": "4",
    },
    {
        "short_description": "Kisoft stock movement doesn't appear in ILOS",
        "description": "hi ILOS seems to no be recording some stock movements done on kisoft. For example on ILOS the positive movement doesn't appear. just the negative one (see attachment) Thanks Francesca",
        "priority": "2",
    },
    {
        "short_description": "SoA master list export file generation",
        "description": "Hi Team, We will receive daily update from SOA_APPLICATION_SUPPORT@HAVI.COM for the export file generation for item master, supplier master and PO list. However, we found out the SKUs list was not export completely and share to the team. Kindly refer the export file attachment shared on 8 Jan. The example SKUs that not export to the file as : 9555021501506, 9555021504293, 9555021513578, 9555021514339",
        "priority": "4",
    },
    {
        "short_description": "SAP/VIM PC4 All company codes impacted - IC4S Validation client stuck impacting Validation Process & Invoice Processing at Krakow",
        "description": "IC4S inbound is stuck again, please solve immediately",
        "priority": "3",
    },
    {
        "short_description": "I can't invoice this package in SAP",
        "description": "I can't invoice this package in SAP",
        "priority": "4",
    },
    {
        "short_description": "Did not recive email in C4H (C4C) in NO",
        "description": "We found out that an email was sent to Kundeservice.no@havi.com. But we did not get it. We have looked in C4C but it is not there. This is critical because we need to be sure that all email arrives. Can this have something to do with both CS SE and CS NO is in the email? We had issue befor that we do not get emails when CS SE is in the same email",
        "priority": "3",
    },
    {
        "short_description": "Haviconnect acessabillity error",
        "description": "When I try to log in, in my Haviconnect, i can access my account, but it shows me error : No business site or store assigned. Please also see attached picture. Just recently my email address got changed, from pavel.jakjan@havi.com to pavels.jakjans@havi.com, so maybe it is an issue",
        "priority": "2",
    },
    {
        "short_description": "Job LOAD_ITEM of application HC_LOAD_JDA_MASTER_AT.is in FAILED state",
        "description": "Job LOAD_ITEM of application HC_LOAD_JDA_MASTER_AT.is in FAILED state.",
        "priority": "3",
    },
]
